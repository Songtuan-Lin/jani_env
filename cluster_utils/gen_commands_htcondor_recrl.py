...
def get_configs_for_benchmark(variant_dir: str, domain_dir: str, shared_args: dict[str, any]) -> list[dict[str, any]]:
    ...
    for model_file in variant_dir.iterdir():
        ...
        assert recover_policy_path.exists(), f"Recovery policy file {recover_policy_path} does not exist."
        assert q_risk_path.exists(), f"Q risk file {q_risk_path} does not exist."
        assert offline_buffer_path.exists(), f"Offline buffer file {offline_buffer_path} does not exist."

        recover_policy_path = condor_dir_prefix / recover_policy_path
        q_risk_path = condor_dir_prefix / q_risk_path
        offline_buffer_path = condor_dir_prefix / offline_buffer_path
        ...
        args = {
            ...
            "recovery_policy_path": recover_policy_path,
            "q_risk_path": q_risk_path,
            "offline_buffer_path": offline_buffer_path,
            "max_steps": shared_args.get("max_steps", 256),
            "total_timesteps": shared_args.get("total_timesteps", 1_000_000),
            "use_oracle": False,
            "wandb_project": f"{jani_name}_clean",
            "wandb_entity": "",
            "experiment_name": f"{jani_name}_{variant_name}_recovery_rl" if variant_name != "models" else f"{jani_name}_recovery_rl",
            "log_dir": log_dir,
